...
from acquisition_plotting.space import Space, Real
...
def generate_nd_data(nx=20, noise_sigma=0):
    samples = [
        _evenly_sample(DATA_SPACE_ND[i][1], nx) for i in range(N)
    ]
...
def result2D():
    ...
    return make_scipy_optmize_result(
        random_in, random_out, DATA_SPACE_2D, gp
    )
...
def resultND():
    ...
    random_out = gp.predict(random_in, return_std=False)
    return make_scipy_optmize_result(
        random_in, random_out, DATA_SPACE_ND, gp
    )


def plot_gp_and_train_data(
        train_in: np.ndarray, train_out: np.ndarray, random_in: np.ndarray,
        random_out: np.ndarray) -> plt.Figure:
    fig, axes = plt.subplots(1, 2, figsize=(10, 5), sharex=True, sharey=True)
    contours = axes[0].contourf(train_in[0], train_in[1], train_out, cmap='plasma')
    axes[0].set_title('True Function')
    axes[1].tricontourf(random_in[:, 0], random_in[:, 1], random_out, cmap='plasma')
